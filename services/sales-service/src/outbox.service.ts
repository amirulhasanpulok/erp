...
export class OutboxService implements OnModuleInit, OnModuleDestroy {
  ...
  async dispatch(outbox: OutboxEventEntity): Promise<void> {
    this.publisher.publish(outbox.eventType, 'sales-service', outbox.payload);
    await this.outboxRepository.markPublished(outbox.id);
  }
  ...
  private async republishPending(): Promise<void> {
    const pending = await this.outboxRepository.listPending(100);
