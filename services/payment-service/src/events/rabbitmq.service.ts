...
export class RabbitMqService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    const queue = process.env.RABBITMQ_QUEUE ?? 'payment-service.events.q';
    const dlq = process.env.RABBITMQ_DLK ?? 'payment-service.events.dlq';
    ...
    await this.channel.bindQueue(queue, exchange, 'ECOM_ORDER_PLACED');
  }
